...
def test_save_graph_to_markdown(mock_open, fresh_graph_builder):
    ...
    save_graph_to_markdown(graph, "test_output.md")
    mock_open.assert_called_once_with("test_output.md", "w")
    mock_open().write.assert_any_call("```mermaid\n")
    mock_open().write.assert_any_call(graph.get_graph().draw_mermaid())
